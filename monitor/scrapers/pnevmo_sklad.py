...
class PnevmoSkladScraper(BaseScraper):
    ...
    def _get_replacement(self, soup: BeautifulSoup) -> str:
        """Модель-замена для снятого товара.

        Тут было две ловушки, из-за которых поле приезжало мусором на КАЖДОЙ
        карточке (замер 11.08: 2 514 из 2 535 со значением «Самарапн-пт»):

        * перебирались в том числе `div`, а контейнер верхнего уровня отдаёт
          текст всей страницы. Слово «аналог» находилось в меню или подвале, и
          регексп возвращал первое попавшееся слово с заглавной — город из
          шапки. Поэтому длинные блоки пропускаем: пометка о замене — это
          фраза, а не страница;
        * `get_text(strip=True)` без разделителя склеивает соседние узлы:
          «Самара» + «пн-пт» → «Самарапн-пт». Разделяем пробелом;
        * модель ищем ПОСЛЕ слова-маркера, а не в начале строки — иначе
          подхватывается то, что стоит перед ним.
        """
        for el in soup.select("p, span"):
            text = el.get_text(" ", strip=True)
            if len(text) > 200:      # это уже не пометка, а кусок страницы
                continue
            low = text.lower()
            hit = next((k for k in ("аналог", "замена", "заменяет") if k in low), None)
            if not hit:
                continue
            # Берём ХВОСТ после маркера целиком, а не первый «длинный токен»:
            # модель почти всегда из нескольких коротких кусков («DMD 30 C 10»),
            # и токен-регексп из них не собирал ничего либо возвращал один
            # бренд. Цифра в хвосте — признак кода модели; она же отсекает
            # мусор вроде «Аналоги запчастей» и «пн-пт».
            tail = text[low.index(hit) + len(hit):].lstrip(" :—–-,.")
            tail = re.split(r"[.;\n]", tail)[0].strip()
            if 2 <= len(tail) <= 60 and any(ch.isdigit() for ch in tail):
                return tail
        return ""
