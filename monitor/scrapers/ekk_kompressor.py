"""Scraper for ekk-kompressor.ru — ONLY Zega brand from mobile electric compressors.

Engine: 1С-Битрикс or custom.
Source: category /catalog/kompressory/peredvizhnye_elektricheskie_kompressory/
        with ?PAGEN_1=N pagination. Only products whose name/brand contains
        "Зега" or "Zega" are emitted (category may contain other brands).

Markup (1C-Bitrix):
- listing cards: .catalog-item a.catalog-item-title or similar
- price:         [itemprop="price"] content  OR  .price
- availability:  .in-stock, .catalog-item-status, [class*="nalichie"]
- specs:         table.props tr, .properties tr
- brand:         filtered to Зега/Zega only
"""
...
BASE = "https://ekk-kompressor.ru"
BRAND = "Зега"
# Brand name variants to recognise in product names/specs
BRAND_MARKERS = ("зега", "zega")
MAX_URLS = int(os.getenv("EKK_KOMPRESSOR_MAX", "0")) or None
MAX_PAGES = 50

CATEGORIES = [
    "https://ekk-kompressor.ru/catalog/kompressory/peredvizhnye_elektricheskie_kompressory/",
]


def _is_zega(text: str) -> bool:
    t = text.lower()
    return any(m in t for m in BRAND_MARKERS)


class EkkKompressorScraper(BaseScraper):
    site = "ekk-kompressor.ru"
    base_url = BASE
    delay_min = 0.8
    delay_max = 1.8
    ...
    def fetch_listing(self, url: str) -> list[str]:
        ...
        for page in range(1, MAX_PAGES + 1):
            ...
            try:
                ...
                logger.warning("[ekk-kompressor] page %d error: %s", page, exc)
                break
            soup = BeautifulSoup(resp.content, "lxml")
            new = 0
            # Collect all product-link candidates first, then filter by brand
            # (title text visible in listing card)
            for a in soup.select(
                "a.catalog-item-title, .catalog-item__title a, "
                ".catalog-item-name a, .item-title a, "
                ".catalog_item a.title, .catalog-section-item a.name"
            ):
                card_text = a.get_text(" ", strip=True)
                href = (a.get("href") or "").split("?")[0].strip()
                if not href:
                    continue
                # If brand is detectable in card text, keep only Zega; if text
                # is empty (icon-only links), fetch all and filter in parse_product.
                if card_text and not _is_zega(card_text):
                    continue
            ...
            if new == 0:
                break
            logger.info("[ekk-kompressor] page %d: %d URLs so far", page, len(urls))
        if MAX_URLS:
            urls = urls[:MAX_URLS]
        logger.info("[ekk-kompressor] %d candidate URLs (Zega-filtered)", len(urls))
        return urls
    ...
    def parse_product(self, url: str) -> Optional[Product]:
        ...
        # Brand guard: skip if product is clearly not Zega
        brand_in_specs = any(
            _is_zega(str(v)) for v in specs.values()
        )
        if not _is_zega(name) and not brand_in_specs:
            return None
